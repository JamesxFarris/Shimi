...
// Auto-bet scan status tracking (legacy — crypto auto-bet is disabled)
let lastScanStatus = {
  timestamp: null,
  marketsScanned: 0,
  marketsWithEdge: 0,
...
app.post('/api/crypto/auto-bet/toggle', (req, res) => {
  res.status(410).json({ success: false, error: 'Crypto auto-betting has been removed. Use weather betting instead.' });
});
...
app.post('/api/kill-switch/activate', (req, res) => {
  ...
  try {
    ...
    globalKillSwitch = true;
    // Stop all weather bet intervals
    for (const [uid] of weatherBetIntervals) {
      stopWeatherBetting(uid);
    }
